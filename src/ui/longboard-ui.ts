...
export function configureLongboardUI(race: Race) {
  ...
  set('.touch-controls [data-control="ArrowDown"]', board ? '脚刹' : '刹车');
  ...
  if (!board) {
    ...
    if (hint) hint.innerHTML = `<kbd>W</kbd> 油门 <kbd>S</kbd> 刹车 <kbd>A</kbd><kbd>D</kbd> 转向 <kbd>SPACE</kbd> <span data-brake-hint>${race.vehicleMode === 'motorcycle' ? '后刹蓄能' : '漂移蓄能'}</span> <kbd>SHIFT</kbd> 氮气`;
    return;
  }

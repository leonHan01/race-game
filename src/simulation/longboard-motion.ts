...
export const longboardPoseKeys = ['stanceYaw', 'switchWeight', 'slide', 'handsDown', 'tuck', 'footbrake', 'push', 'pushPhase'] as const;
export const idleLongboardPose = (): LongboardPose => ({ stanceYaw: 0, switchWeight: 0, slide: 0, handsDown: 0, tuck: 0, footbrake: 0, push: 0, pushPhase: 0 });
...
export class LongboardMotion {
  ...
  update(dt: number, input: { speed: number; steering: number; slide: boolean; standup: boolean; brake: boolean; tuck: boolean; push: boolean }) {
    ...
    if (this.switchTime > 0) {
      ...
      this.pose.stanceYaw = this.switchStart + (this.switchTarget - this.switchStart) * smooth(progress);
      ...
      if (sliding) {
        if (this.style === 'none') this.direction = Math.abs(steering) > 0.1 ? Math.sign(steering) : this.stance === 'regular' ? 1 : -1;
    ...
    const hands = this.style === 'hands-down' ? smooth((this.heldTime - 0.06) / 0.24) : 0;
    ...
    for (const key of ['slide', 'handsDown', 'tuck', 'footbrake', 'push'] as const) {
      this.pose[key] += (targets[key] - this.pose[key]) * (1 - Math.exp(-dt * 11));
    }
